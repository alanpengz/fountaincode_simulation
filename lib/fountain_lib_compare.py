...
def test_ew_feedback_fountain():
    ...
    for f in file_list:
        ...
        res.to_csv(os.path.join(SIM_PATH, '论文多反馈UEP_LT方案/RSD/K_10/feedback_EW_K' + '_'+ str(K) + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
    ...
    avg_res.to_csv(os.path.join(SIM_PATH, '论文多反馈UEP_LT方案/RSD/K_10/feedback_EW_avgs' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
