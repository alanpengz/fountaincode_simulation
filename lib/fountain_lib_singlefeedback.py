...
def robust_process_randChunkNums(chunks):
    size = robust_soliton(len(chunks)).__next__()
    return [ii for ii in random.sample(chunks, size)]
...
class Droplet:
    ...
    def robust_process_ChunkNums(self):
        random.seed(self.seed)
        np.random.seed(self.seed)
        return robust_process_randChunkNums(self.process)
...
class Fountain(object):
    ...
    def droplet(self):
        ...
        if not self.all_at_once:
            ...
            self.chunk_selected = robust_process_randChunkNums(self.chunk_process)
...
class EW_Fountain(Fountain):
    ...
    def droplet(self):
        ...
        if not self.all_at_once:
            ...
            self.chunk_selected = self.EW_robust_process_RandChunkNums()
    ...
    # 新加
    def EW_robust_process_RandChunkNums(self):
        return robust_process_randChunkNums(self.chunk_process)
...
class EW_Droplet(Droplet):
    ...
    def robust_process_ChunkNums(self):
        random.seed(self.seed)
        np.random.seed(self.seed)
        return self.ower.EW_robust_process_RandChunkNums()
...
class Glass:
    ...
    def addDroplet(self, drop):
        ...
        entry = [drop.robust_chunkNums(), drop.data] if not self.all_at_once else [drop.robust_process_ChunkNums(), drop.data] 
    ...
    def getProcess(self):
        ...
        for chunk in self.chunks:
            if chunk is None:
                process.append(idx)
            idx += 1
        finished_lib = self.get_finished_lib()
        return process.extend(finished_lib)

    def get_finished_lib(self):
        idx = round(0.15*self.num_chunks)
        finished_lib = []
        while idx < self.num_chunks:
            if self.chunks[idx] != None:
                finished_lib.append(idx)
        finished_lib_len = len(finished_lib)
        unfinished_lib_len = self.num_chunks - idx - finished_lib_len
        select_num = round(unfinished_lib_len/10)
        return  [ii for ii in np.random.choice(finished_lib, select_num, False)]
...
def test_ew_feedback_fountain():
    ...
    for f in file_list:
        ...
        res = pd.DataFrame({'num_chunks':num_chunks_list, 
            'times':times_list, 
            'drop_num_used':drop_num_used_list})
        res.to_csv(os.path.join(SIM_PATH, 'RSD_EW_LT/drop改ew_drop后/feedback_once/feedback_EW_K' + '_'+ str(K) + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
    ...
    avg_res = pd.DataFrame({'K': [ii.split('.')[0] for ii in suffix_list], 
            'avgs':avg_drops_list, 'feedback_packet_avgs':avg_acknums_list})
    avg_res.to_csv(os.path.join(SIM_PATH, 'RSD_EW_LT/drop改ew_drop后/feedback_once/feedback_EW_avgs' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
