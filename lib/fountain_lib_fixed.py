...
class Fountain(object):
    ...
    def droplet(self):
        ...
        if not self.all_at_once:
            self.chunk_selected = fixed_degree_randChunkNums(self.num_chunks)
        else:
...
class EW_Fountain(Fountain):
    ...
    def __init__(self, data, chunk_size, ew_process=[], seed=None, w1_size=0.15, w1_pro=0.084):
        ...
        self.w2_random_chunk_gen = fixed_degree_distribution_func()
    ...
    def droplet(self):
        self.updateSeed()
        if not self.all_at_once:
            self.chunk_selected = self.EW_fixed_RandChunkNums(self.num_chunks)
        else:
    ...
    def EW_fixed_RandChunkNums(self, num_chunks):
        '''扩展窗的不同在这里'''
...
class EW_Droplet(Droplet):
    ...
    def fixed_degree_chunkNums(self):
        random.seed(self.seed)
        np.random.seed(self.seed)
        return self.ower.EW_fixed_RandChunkNums(self.num_chunks)
...
class Glass:
    ...
    def addDroplet(self, drop):
        ...
        entry = [drop.fixed_degree_chunkNums(), drop.data] if not self.all_at_once else [drop.all_at_once_chunkNums(), drop.data] 
...
def test_LT_fountain():
    ...
    for f in file_list:
        ...
        while times < 1000:
            ...
            logging.info("fixed_degree K=" + str(fountain.num_chunks) +" times: " + str(times) + 'done, receive_drop_used: ' + str(glass.dropid))
            times += 1
    ...
    avg_res = pd.DataFrame({'K': [ii.split('.')[0] for ii in suffix_list], 
            'avgs':avg_drops_list})
    avg_res.to_csv(os.path.join(SIM_PATH, 'fixed_degree_LT(K-avgs)' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
...
def test_LT_feedback_fountain():
    ...
    for f in file_list:
        ...
        while times < 100:
            ...
            logging.info("fixed_degree_feedback K=" + str(fountain.num_chunks) +" times: " + str(times) + 'done, receive_drop_used: ' + str(glass.dropid))
            times += 1

        res = pd.DataFrame({'num_chunks':num_chunks_list, 
            'times':times_list, 
            'drop_num_used':drop_num_used_list})
        res.to_csv(os.path.join(SIM_PATH, 'fixed/0.8K_5/feedback_K' + '_'+ str(K) + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
    ...
    avg_res.to_csv(os.path.join(SIM_PATH, 'fixed/0.8K_5/fixed_degree_feedback_LT(K-avgs)' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
...
def test_ew_fountain():
    ...
    for f in file_list:
        ...
        while times < 100:
            ...
            logging.info("fixed_degree_EW K=" + str(fountain.num_chunks) +" times: " + str(times) + 'done, receive_drop_used: ' + str(glass.dropid))
            times += 1
    ...
    avg_res = pd.DataFrame({'K': [ii.split('.')[0] for ii in suffix_list], 
            'avgs':avg_drops_list})
    avg_res.to_csv(os.path.join(SIM_PATH, 'fixed_degree_EW_LT(K-avgs)' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
...
def test_ew_feedback_fountain():
    ...
    for f in file_list:
        ...
        while times < 100:
            ...
            logging.info("fixed_degree_EW_feedback K=" + str(fountain.num_chunks) +" times: " + str(times) + 'done, receive_drop_used: ' + str(glass.dropid))
            times += 1

        res = pd.DataFrame({'num_chunks':num_chunks_list, 
            'times':times_list, 
            'drop_num_used':drop_num_used_list})
        res.to_csv(os.path.join(SIM_PATH, 'fixed/0.8K_5/feedback_EW_K' + '_'+ str(K) + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
    ...
    avg_res = pd.DataFrame({'K': [ii.split('.')[0] for ii in suffix_list], 
            'avgs':avg_drops_list, 'feedback_packet_avgs':avg_acknums_list})
    avg_res.to_csv(os.path.join(SIM_PATH, 'fixed/0.8K_5/fixed_degree_feedback_EW_LT(K-avgs)' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
