...
def test_ew_feedback_fountain():
    ...
    for f in file_list:
        ...
        while times < 100:
            ...
            while not glass.isDone():
                ...
                if(glass.is_w1_done(0.15)):
                    ...
                    # 之后每10个包反馈进度
                    if((glass.dropid - glass.w1_recv_n)%10 == 0):
                        ack_num += 1
        ...
        res.to_csv(os.path.join(SIM_PATH, '论文多反馈UEP_LT方案/mfixed/K_10/feedback_EW_K' + '_'+ str(K) + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
    ...
    avg_res.to_csv(os.path.join(SIM_PATH, '论文多反馈UEP_LT方案/mfixed/K_10/mfixed_degree_feedback_EW_LT(K-avgs)' + '_' + time.asctime().replace(' ', '_').replace(':', '_') + '.csv'),  mode='a')
